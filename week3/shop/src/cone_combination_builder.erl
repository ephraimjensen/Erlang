...
cone_combinations(Top_flavors, Bottom_flavors) when not is_list(Top_flavors) -> fail;
...
most_popular_combinations(Count, List) when length(List) < Count-> fail;
most_popular_combinations(Count, List)->
	Sorted_list = lists:sort(List),
	{Top_numbers, Other_numbers} = lists:split(Count, Sorted_list),
	Top_numbers.
