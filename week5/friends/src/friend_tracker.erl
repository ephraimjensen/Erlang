...
run(Friend_list) ->
	receive 
		{From, {Command, Data}} when Command == add ->
			From ! received,
			run([Data|Friend_list]);

		{From, {Command, Data}} when Command == has_friend->
			From ! lists:member(Data, Friend_list),
			run([Friend_list]);

		{From, {Command, Data}} when Command == has_friends->
			From ! lists:all(fun(Elem) -> lists:member(Elem, Friend_list) end, Data),
			run([Friend_list]);

		{From, {Command, Data}} when Command == remove ->
			From ! received,
